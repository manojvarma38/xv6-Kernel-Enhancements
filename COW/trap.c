...
void usertrap(void)
{
  ...
  else if (r_scause() == 15)
  {
    ...
    // Check if the address is valid and the fault is a legitimate COW fault
    if (va >= MAXVA)
    {
      setkilled(p);
    }
    else
    {
      ...
      {
        // Not a valid COW page, it's a real protection fault
        setkilled(p);
      }
      else
      {
        // It is a valid COW fault, proceed with the copy
        ...
        else
        {
          ...
          // Map the new private, writable page
          if (mappages(p->pagetable, PGROUNDDOWN(va), PGSIZE, (uint64)mem, (flags & ~PTE_COW) | PTE_W) != 0)
          {
            kfree(mem);
            setkilled(p);
          }
          // The process no longer uses the old page, so decrement its ref count
          ref_dec(pa);
        }
      }
    }
